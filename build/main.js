...
const LUBA_PRO_PRODUCT_KEYS = /* @__PURE__ */ new Set(["a1mb8v6tnAa", "a1pHsTqyoPR"]);
...
class Mammotion extends utils.Adapter {
  ...
  async reapplyTaskSettingsAfterStart(deviceKey, trigger) {
    ...
    try {
      ...
      this.log.info(
        `Start-Reapply (${trigger}) f\xFCr ${ctx.deviceName || ctx.iotId}: H\xF6he ${cutHeightMm} mm, Speed ${mowSpeedMs} m/s.`
      );
      ...
      this.log.warn(`Start-Reapply (${trigger}) fehlgeschlagen f\xFCr ${ctx.deviceName || ctx.iotId}: ${msg}`);
    }
  }
  ...
  async handleMqttMessage(topic, payload) {
    ...
    if (!deviceKey) {
      this.log.debug(`[MQTT] Kein deviceKey f\xFCr pk=${productKey} dn=${recordDeviceName} iotId=${payloadIotId}`);
      return;
    }
    const ctx = this.deviceContexts.get(deviceKey);
    ...
    if (typeof protoContent === "string") {
      ...
      if (areas && areas.length > 0) {
        ...
        if (hashIds && hashIds.length > 0 && ctx) {
          ...
          this.log.debug(`[MQTT] protoContent enth\xE4lt keine Zone-Namen / kein HashListAck`);
        }
      }
    } else {
      this.log.debug(`[MQTT] Kein protoContent gefunden. params.value=${((_na = JSON.stringify(params == null ? void 0 : params.value)) != null ? _na : "(none)").substring(0, 200)}`);
    }
  }
  ...
  async refreshSessionAndDeviceCache() {
    ...
    if (records.length && (!this.mqttClient || !this.mqttClient.connected)) {
      try {
        ...
        this.log.debug(`JWT-MQTT-Credentials nicht verf\xFCgbar (${this.extractAxiosError(err)}), AEP-Fallback bleibt aktiv.`);
        if (this.mqttClient) {
          this.mqttClient.removeAllListeners();
          this.mqttClient.end(true);
          this.mqttClient = null;
  ...
  async fetchLegacyProperties(session, iotId) {
    const load = async (activeSession, forceSessionRefresh) => {
      ...
      this.log.debug(`[PROPS] Property keys f\xFCr ${iotId}: ${Object.keys(response.data || {}).join(", ")}`);
      return response.data || null;
    };
    try {
...
${url}`;
  ...
  async sendAreaNameListRequest(context) {
    var _a, _b;
    this.log.debug(`[AREA-REQ] Sende sub_cmd=3 (AppGetAllAreaHashName) f\xFCr ${context.deviceName || context.iotId}`);
    const result3 = await this.executeEncodedContentCommand(
      context,
      "area-name-list-v3",
      (_session, ctx) => this.buildAreaNameListContent(_session, ctx, 3)
    );
    this.log.debug(`[AREA-REQ] sub_cmd=3 Response (len=${(_a = result3 == null ? void 0 : result3.length) != null ? _a : 0}): ${result3 == null ? void 0 : result3.substring(0, 100)}`);
    ...
    const result0 = await this.executeEncodedContentCommand(
      context,
      "area-name-list",
      (_session, ctx) => this.buildAreaNameListContent(_session, ctx, 0)
    );
    this.log.debug(`[AREA-REQ] sub_cmd=0 Response (len=${(_b = result0 == null ? void 0 : result0.length) != null ? _b : 0}): ${result0 == null ? void 0 : result0.substring(0, 100)}`);
  ...
  async requestAreaNamesForHashes(context, hashIds) {
    ...
    this.log.info(`[ZONE] ${areaHashes.length} M\xE4hzonen (type=0) von ${hashIds.length} Hashes erkannt`);
    const areas = areaHashes.length > 0 ? areaHashes.map((h, i) => ({ name: `Area ${i + 1}`, hash: h })) : hashIds.map((h, i) => ({ name: `Area ${i + 1}`, hash: h }));
    await this.updateZoneStates(context.key, areas);
  }
  /**
  ...
  resolveCommDataAck(deviceKey, protoBase64) {
    ...
    try {
      const buf = Buffer.from(protoBase64, "base64");
      const lubaFields = this.decodeProtoFields(buf);
      for (const navBuf of (_a = lubaFields.get(11)) != null ? _a : []) {
        if (!(navBuf instanceof Buffer)) continue;
        const navFields = this.decodeProtoFields(navBuf);
        for (const ackBuf of (_b = navFields.get(33)) != null ? _b : []) {
          if (!(ackBuf instanceof Buffer)) continue;
          const ackFields = this.decodeProtoFields(ackBuf);
          const type = Number((_d = (_c = ackFields.get(5)) == null ? void 0 : _c[0]) != null ? _d : 0n);
          const hashVal = (_e = ackFields.get(6)) == null ? void 0 : _e[0];
  ...
  async requestAreaNamesForAllDevices() {
    for (const ctx of this.deviceContexts.values()) {
      try {
        ...
        this.log.debug(`Area-Name-Anfrage fehlgeschlagen f\xFCr ${ctx.deviceName || ctx.iotId}: ${this.extractAxiosError(err)}`);
      }
    }
  }
  ...
  async handleRequestAreaNames(deviceKey, localId) {
    const ctx = this.deviceContexts.get(deviceKey);
    if (!ctx) {
      await this.setStateChangedAsync(`devices.${deviceKey}.commands.lastError`, `Unbekanntes Ger\xE4t: ${deviceKey}`, true);
      await this.setStateChangedAsync(localId, false, true);
      return;
    }
    try {
      await this.sendAreaNameListRequest(ctx);
      await this.setStateChangedAsync(localId, false, true);
      this.log.info(`Area-Name-Liste angefordert f\xFCr ${ctx.deviceName || ctx.iotId}.`);
    } catch (err) {
      const msg = this.extractAxiosError(err);
      await this.setStateChangedAsync(`devices.${deviceKey}.commands.lastError`, msg, true);
      await this.setStateChangedAsync(localId, false, true);
      this.log.warn(`Area-Name-Anfrage fehlgeschlagen f\xFCr ${ctx.deviceName}: ${msg}`);
    }
  }
  ...
  tryParseAreaHashNames(protoBase64) {
    ...
    try {
      ...
      for (const navBuf of navBufs) {
        ...
        for (const areaListBuf of (_a = navFields.get(61)) != null ? _a : []) {
          ...
          for (const hashNameBuf of (_b = areaListFields.get(2)) != null ? _b : []) {
            ...
            if (hash !== void 0 && !(hash instanceof Buffer) && nameBuf instanceof Buffer) {
              ...
              if (name) areas.push({ name, hash });
            }
          }
        }
        for (const ackBuf of (_e = navFields.get(33)) != null ? _e : []) {
          if (!(ackBuf instanceof Buffer)) continue;
          const ackFields = this.decodeProtoFields(ackBuf);
  ...
  tryParseNavGetHashListAck(protoBase64, deviceKey) {
    ...
    try {
      ...
      for (const navBuf of navBufs) {
        ...
        for (const ackBuf of (_a = navFields.get(31)) != null ? _a : []) {
          if (!(ackBuf instanceof Buffer)) continue;
          const ackFields = this.decodeProtoFields(ackBuf);
  ...
  async handleStartZones(deviceKey, localId) {
    ...
    try {
      ...
      if (!(areasJsonState == null ? void 0 : areasJsonState.val)) {
        throw new Error('Keine Zonen bekannt. Bitte erst "requestAreaNames" ausf\xFChren.');
      }
      ...
      await this.setStateChangedAsync(`devices.${deviceKey}.commands.lastTimestamp`, now, true);
      await this.setStateChangedAsync(localId, false, true);
      this.log.info(`startZones f\xFCr ${ctx.deviceName || ctx.iotId}: ${areaHashes.length} Zone(n) gesendet.`);
      await this.requestIotSync(ctx);
      await this.refreshTelemetryAfterCommand();
    } catch (err) {
      const msg = this.extractAxiosError(err);
  ...
  async handleStartAllZones(deviceKey, localId) {
    ...
    try {
      ...
      await this.setStateChangedAsync(`devices.${deviceKey}.commands.lastTimestamp`, now, true);
      await this.setStateChangedAsync(localId, false, true);
      this.log.info(`startAllZones f\xFCr ${ctx.deviceName || ctx.iotId}: ${areaHashes.length} Zone(n) gesendet.`);
      await this.requestIotSync(ctx);
      await this.refreshTelemetryAfterCommand();
    } catch (err) {
      const msg = this.extractAxiosError(err);
  ...
  async handleStartSingleZone(deviceKey, zoneSanitizedName, localId) {
    ...
    try {
      ...
      if (!(hashState == null ? void 0 : hashState.val)) {
        throw new Error(`Kein Hash f\xFCr Zone "${zoneSanitizedName}" gefunden.`);
      }
      ...
      await this.setStateChangedAsync(`devices.${deviceKey}.commands.lastTimestamp`, now, true);
      await this.setStateChangedAsync(localId, false, true);
      this.log.info(`startSingleZone f\xFCr ${ctx.deviceName || ctx.iotId}: Zone "${zoneSanitizedName}" gesendet.`);
      await this.requestIotSync(ctx);
      await this.refreshTelemetryAfterCommand();
    } catch (err) {
      const msg = this.extractAxiosError(err);
  ...
  async handlePayloadCommand(deviceKey, localId, jsonStr) {
    ...
    try {
      ...
      if (payloadDeviceCommand) {
        ...
        this.log.info(`Payload-Command f\xFCr ${ctx.deviceName || ctx.iotId} gesendet (${payloadDeviceCommand}).`);
        await this.requestIotSync(ctx);
        await this.refreshTelemetryAfterCommand();
        return;
      }
      ...
      this.log.info(`Payload-Command f\xFCr ${ctx.deviceName || ctx.iotId} gesendet (${executeStart ? "startRoute" : routeMode}).`);
      await this.requestIotSync(ctx);
      await this.refreshTelemetryAfterCommand();
    } catch (err) {
      const msg = this.extractAxiosError(err);
