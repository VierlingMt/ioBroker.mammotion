...
class Mammotion extends utils.Adapter {
    ...
    private async handleMqttMessage(topic: string, payload: Buffer): Promise<void> {
        ...
        const protoContent = params?.value?.content ?? data?.value?.content;
    ...
    private async invokeTaskControlCommandLegacy(session: AuthSession, context: DeviceContext, content: string): Promise<string> {
        ...
        try {
            ...
            this.authFailureSince = 0;
            await this.setStateChangedAsync('info.connection', true, true);
            return result?.messageId || result?.data || 'ok';
        } catch (err) {
            const msg = this.extractAxiosError(err).toLowerCase();
            if (
                !msg.includes('token') &&
        ...
        this.authFailureSince = 0;
        await this.setStateChangedAsync('info.connection', true, true);
        return retry?.messageId || retry?.data || 'ok';
    }
    ...
    private buildRoutePathOrderBytes(settings: RouteSettings): Buffer {
    ...
    private async ensureDeviceStateObjects(channelId: string): Promise<void> {
        ...
        await this.setObjectNotExistsAsync(
            ...
            this.createWritableNumberState('Route job version', 'level', 0, { min: 0, max: 1000000, step: 1 }),
        );
        await this.setObjectNotExistsAsync(
            `${channelId}.commands.routeJobId`,
            this.createWritableNumberState('Route job id', 'level', 0, { min: 0, max: 9_000_000_000_000_000, step: 1 }),
        );
        await this.setObjectNotExistsAsync(
    ...
    private createCommandState(name: string): ioBroker.SettableObject {
        return {
            type: 'state',
            common: {
                name,
    ...
    private createWritableBooleanState(name: string, def: boolean): ioBroker.SettableObject {
        return {
            type: 'state',
            common: {
                name,
    ...
    private createWritableStringState(name: string, role: string, def: string): ioBroker.SettableObject {
        return {
            type: 'state',
            common: {
                name,
